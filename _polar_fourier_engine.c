 ...
 *
 * Engine C: Polar CPU ¡ª 180¡ã FOV, zenith-angle ¡Á azimuth grid,
 *            OpenMP multi-core parallelism (up to 8 cores).
 *
 * Mathematical foundation:
 *   I_D(l,m) = ¦²_k V_k ¡¤ exp{+2¦Ði [u_k¡¤l + v_k¡¤m + w_k¡¤(n - 1)]}
 *
 * where:
 *   l = cos(alt) * sin(az)    (East direction cosine)
 *   m = cos(alt) * cos(az)    (North direction cosine)
 *   n = sin(alt)              (Zenith direction cosine)
 *   alt = 90¡ã - zenith_angle
 *
 * The grid is in polar coordinates:
 *   - Radial axis:    zenith angle ¦Æ ¡Ê [0¡ã, 90¡ã]  (n_radial bins)
 *   - Azimuthal axis: azimuth A ¡Ê [0¡ã, 360¡ã)      (n_azimuthal bins)
 *
    ...
    int num_threads)
{
    ...
    /* Zenith angle: linear from 0¡ã to 90¡ã (cell centres) */
    for (int i = 0; i < n_radial; i++) {
        double zeta_deg = (i + 0.5) * 90.0 / n_radial;
        zeta_rad[i] = zeta_deg * deg_to_rad;
    }

    /* Azimuth: linear from 0¡ã to 360¡ã (cell centres) */
...
#pragma omp parallel for schedule(static)
    for (int i = 0; i < n_radial; i++) {
        ...
        double alt = (M_PI / 2.0) - zeta;   /* altitude = 90¡ã - zenith */
        ...
        for (int j = 0; j < n_azimuthal; j++) {
            ...
            for (int k = 0; k < n_baselines; k++) {
                /* Phase ¦Õ_k = 2¦Ð [u_k¡¤l + v_k¡¤m + w_k¡¤(n - 1)] */
...
/*  0 */ _CFFI_OP(_CFFI_OP_FUNCTION, 2), // int()(double *, int, int, double const *, double const *, double const *, double const *, double const *, int, double, int, int)
/*  1 */ _CFFI_OP(_CFFI_OP_POINTER, 10), // double *
/*  2 */ _CFFI_OP(_CFFI_OP_PRIMITIVE, 7), // int
/*  3 */ _CFFI_OP(_CFFI_OP_PRIMITIVE, 7),
/*  4 */ _CFFI_OP(_CFFI_OP_POINTER, 10), // double const *
...
static int _cffi_d_compute_dirty_image_polar(double * x0, int x1, int x2, double const * x3, double const * x4, double const * x5, double const * x6, double const * x7, int x8, double x9, int x10, int x11)
{
  return compute_dirty_image_polar(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11);
}
#ifndef PYPY_VERSION
static PyObject *
_cffi_f_compute_dirty_image_polar(PyObject *self, PyObject *args)
{
  double * x0;
  int x1;
  int x2;
  ...
  if (!PyArg_UnpackTuple(args, "compute_dirty_image_polar", 12, 12, &arg0, &arg1, &arg2, &arg3, &arg4, &arg5, &arg6, &arg7, &arg8, &arg9, &arg10, &arg11))
    return NULL;
  ...
  _cffi_restore_errno();
  { result = compute_dirty_image_polar(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11); }
  _cffi_save_errno();
...
#else
#  define _cffi_f_compute_dirty_image_polar _cffi_d_compute_dirty_image_polar
#endif

static const struct _cffi_global_s _cffi_globals[] = {
  { "compute_dirty_image_polar", (void *)_cffi_f_compute_dirty_image_polar, _CFFI_OP(_CFFI_OP_CPYTHON_BLTN_V, 0), (void *)_cffi_d_compute_dirty_image_polar },
};
...
#ifdef PYPY_VERSION
PyMODINIT_FUNC
_cffi_pypyinit__polar_fourier_engine(const void *p[])
{
...
#  ifdef _MSC_VER
     PyMODINIT_FUNC
#  if PY_MAJOR_VERSION >= 3
     PyInit__polar_fourier_engine(void) { return NULL; }
#  else
     init_polar_fourier_engine(void) { }
#  endif
#  endif
#elif PY_MAJOR_VERSION >= 3
PyMODINIT_FUNC
PyInit__polar_fourier_engine(void)
{
  return _cffi_init("_polar_fourier_engine", 0x2601, &_cffi_type_context);
}
#else
PyMODINIT_FUNC
init_polar_fourier_engine(void)
{
  _cffi_init("_polar_fourier_engine", 0x2601, &_cffi_type_context);
}
#endif
